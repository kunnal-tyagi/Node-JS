        ...
        {requests.map((plot)=>{
         ...
         return (
           <div key={_id} className='flex m-4 p-4 rounded-lg bg-base-200'>
            ...
            <div className='my-7 '>
                <button className="btn btn-outline btn-success " onClick={()=>{ReviewRequest("accepted",plot._id)}}>Accept</button>
                <button className="btn btn-outline btn-error mx-3" onClick={()=>{ReviewRequest("rejected",plot._id)}}>Reject</button>
            </div>
          </div>
         )
        })}
